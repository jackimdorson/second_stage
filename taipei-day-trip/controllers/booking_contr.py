...
from schemas.booking_schemas import ReqBookingSchema, ReqCartSchema, ReqCartListSchema
...
async def get_cart_info(authorization: str | None = fastapi.Header(None)) -> ReqCartListSchema: #名前衝突する為jwtと命名付けしない
    if (authorization is None):
        raise fastapi.HTTPException(status_code = 403, detail = "未登入系統，拒絕存取") #viewではなく、検証のためコントローラーに記述
...
async def delete_cart_info(item_id: ReqBookingSchema, authorization: str | None = fastapi.Header(None)) -> ResOkSchema | None: #Noneが帰る可能性がある為。
    if (authorization is None):
        raise fastapi.HTTPException(status_code = 403, detail = "未登入系統，拒絕存取")
